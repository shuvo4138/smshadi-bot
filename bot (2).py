...
def fetch_all_recent_otps():
    """Fetch recent OTPs directly from Panel API"""
    try:
        now = datetime.now()
        ...
        if resp.status_code != 200:
            logger.error(f"Panel API HTTP: {resp.status_code}")
            return []
        
        data = resp.json()
        rows = data.get("aaData", [])
        logger.info(f"📡 Panel API: {len(rows)} rows")
...
def panel_login() -> bool:
    """
    Login to panel — handles math captcha only.
    reCAPTCHA is present on the page but NOT validated server-side
    IF we already have a valid PHPSESSID. So we first check if current
    session is still alive; only re-login if it's dead.
    """
    global PANEL_PHPSESSID, PANEL_SESSKEY

    # ── Step 1: Check if current session still valid ──
    if PANEL_PHPSESSID:
        try:
            test = requests.get(
                PANEL_API_URL,
                params={"fdate1": datetime.now().strftime("%Y-%m-%d 00:00:00"),
                        "fdate2": datetime.now().strftime("%Y-%m-%d 23:59:59"),
                        "sesskey": PANEL_SESSKEY, "sEcho": "1",
                        "iColumns": "9", "iDisplayStart": "0", "iDisplayLength": "1"},
                cookies={"PHPSESSID": PANEL_PHPSESSID},
                timeout=8
            )
            if test.status_code == 200 and "aaData" in test.text:
                logger.info("✅ Existing session still valid, skip re-login")
                return True
        except Exception:
            pass
        logger.info("⚠️ Session expired, attempting re-login...")

    # ── Step 2: Try login (math captcha only, skip reCAPTCHA) ──
    try:
        s = requests.Session()
        s.headers.update({"User-Agent": "Mozilla/5.0 (Linux; Android 10) Chrome/120"})
        resp = s.get("http://185.2.83.39/ints/login", timeout=10)

        a, b = extract_captcha(resp.text)
        if a is None:
            logger.error("❌ Math CAPTCHA not found")
            return False

        captcha_answer = str(a + b)
        logger.info(f"🔢 CAPTCHA: {a}+{b}={captcha_answer}")

        # Send without g-recaptcha-response — some panels don't validate it
        login_resp = s.post(
            "http://185.2.83.39/ints/login",
            data={
                "username": DASHBOARD_USER,
                "password": DASHBOARD_PASS,
                "capt":     captcha_answer,   # field name from browser inspection
                "captcha":  captcha_answer,   # fallback
            },
            timeout=10,
        ...
        if "PHPSESSID" in s.cookies:
            new_sess = s.cookies["PHPSESSID"]

            # Auto-fetch sesskey from dashboard page
            try:
                dash = s.get(
                    "http://185.2.83.39/ints/agent/SMSDashboard",
                    cookies={"PHPSESSID": new_sess},
                    timeout=8
                )
                sk_match = re.search(r'sesskey=([A-Za-z0-9+/=]+)', dash.url + dash.text)
                if sk_match:
                    PANEL_SESSKEY = sk_match.group(1)
                    logger.info(f"🔑 Auto sesskey: {PANEL_SESSKEY}")
                else:
                    logger.warning("⚠️ sesskey not found in dashboard, keeping old value")
            except Exception as e:
                logger.warning(f"⚠️ sesskey fetch error: {e}")

            # Verify session works
            verify = s.get(PANEL_API_URL,
                params={"fdate1": datetime.now().strftime("%Y-%m-%d 00:00:00"),
                        "fdate2": datetime.now().strftime("%Y-%m-%d 23:59:59"),
                        "sesskey": PANEL_SESSKEY, "sEcho": "1",
                        "iColumns": "9", "iDisplayStart": "0", "iDisplayLength": "1"},
                cookies={"PHPSESSID": new_sess}, timeout=8)

            if verify.status_code == 200 and "aaData" in verify.text:
                PANEL_PHPSESSID = new_sess
                logger.info(f"✅ Panel login OK! PHPSESSID={PANEL_PHPSESSID[:12]}...")
                return True
            else:
                logger.error("❌ Got cookie but session invalid (reCAPTCHA blocking)")
                return False
        else:
            logger.error("❌ No PHPSESSID in response — login failed")
            return False
...
async def set_session_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        await update.message.reply_text(
            "📋 *Session Update করার নিয়ম:*\n\n"
            "1️⃣ Phone এ Brave/Chrome এ এই URL এ login করো:\n"
            "`http://185.2.83.39/ints/login`\n\n"
            "2️⃣ Login হলে address bar এ দাও:\n"
            "`javascript:alert(document.cookie)`\n\n"
            "3️⃣ PHPSESSID value copy করো\n\n"
            "4️⃣ এখানে পাঠাও:\n"
            "`/setsession YOUR_PHPSESSID_HERE`\n\n"
            f"⏱ Current session: `{PANEL_PHPSESSID[:15] if PANEL_PHPSESSID else 'None'}...`",
            parse_mode="Markdown"
        )
        return
    ...
    # Auto-fetch sesskey from dashboard
    try:
        s = requests.Session()
        dash = s.get(
            "http://185.2.83.39/ints/agent/SMSDashboard",
        ...
        if sk_match:
            PANEL_SESSKEY = sk_match.group(1)
            logger.info(f"🔑 Auto sesskey updated: {PANEL_SESSKEY}")
    except Exception as e:
        logger.warning(f"sesskey auto-fetch failed: {e}")
    ...
    try:
        test = requests.get(
            ...
            params={"fdate1": datetime.now().strftime("%Y-%m-%d 00:00:00"),
                    "fdate2": datetime.now().strftime("%Y-%m-%d 23:59:59"),
                    "sesskey": PANEL_SESSKEY, "sEcho": "1",
                    "iColumns": "9", "iDisplayStart": "0", "iDisplayLength": "5"},
            cookies={"PHPSESSID": PANEL_PHPSESSID},
            timeout=8
        )
        if test.status_code == 200 and "aaData" in test.text:
            ...
            await update.message.reply_text(
                f"✅ *Session Valid!*\n\n"
                f"🔑 PHPSESSID: `{PANEL_PHPSESSID[:20]}...`\n"
                f"📊 Panel এ `{rows}` OTP row পাওয়া গেছে\n"
                f"🤖 Bot এখন কাজ করবে!",
                parse_mode="Markdown"
            )
        else:
            await update.message.reply_text(
                f"⚠️ *Session Set হয়েছে কিন্তু verify হয়নি*\n\n"
                f"PHPSESSID: `{PANEL_PHPSESSID[:20]}...`\n"
                f"Status: `{test.status_code}`\n\n"
                f"আবার login করে নতুন session দাও।",
                parse_mode="Markdown"
            )
    except Exception as e:
        await update.message.reply_text(
            f"✅ Session set হয়েছে (verify করা যায়নি)\n`{e}`",
            parse_mode="Markdown"
        )

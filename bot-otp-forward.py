...
def fetch_all_recent_otps():
    """Fetch recent OTPs from CR API - last 24 hours"""
    try:
        now = datetime.now()
        ...
        if resp.status_code == 200:
            ...
            logger.error(f"❌ CR API 403 - Token may be invalid or expired")
        else:
...
def check_api_status():
    ...
    try:
        resp = requests.get(
            CR_API_URL,
            params={"token": CR_API_TOKEN, "records": 1},
            timeout=5
        )
        if resp.status_code == 200:
            data = resp.json()
...
def extract_captcha(html: str) -> tuple:
    """Extract CAPTCHA question from login page"""
    try:
        # Try pattern 1: "What is 5 + 0"
        match = re.search(r'What is (\d+)\s*\+\s*(\d+)', html, re.IGNORECASE)
        if match:
            return int(match.group(1)), int(match.group(2))
        
        # Try pattern 2: "(\d+)\s*\+\s*(\d+)" anywhere
...
async def login_panel(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    """Login to panel and get token"""
    try:
        session = requests.Session()
        
        # Step 1: Get CAPTCHA
        resp = session.get("http://185.2.83.39/ints/login", timeout=10)
        a, b = extract_captcha(resp.text)
        
        if a is None or b is None:
            await context.bot.send_message(user_id, "❌ CAPTCHA extract failed")
            return False
        ...
        if "PHPSESSID" in session.cookies:
            await context.bot.send_message(user_id, "✅ *Panel login successful!*\n\n🔑 Ready to fetch OTPs", parse_mode="Markdown")
            logger.info("✅ Panel login successful")
            return True
        else:
            await context.bot.send_message(user_id, "❌ Login failed - wrong credentials")
            return False
            
    except Exception as e:
        logger.error(f"❌ Login error: {e}")
        await context.bot.send_message(user_id, f"❌ Login error: {e}")
        return False
    """On startup, load existing OTPs into cache so they don't get resent"""
    global otp_cache
    rows = fetch_all_recent_otps()
    count = 0
    for row in rows:
        try:
            ...
            pass
    logger.info(f"✅ Preloaded {count} OTPs into cache")
...
async def poll_otps(context):
    """Background task to check for new OTPs via CR API and forward to channel + user inbox"""
    
    rows = fetch_all_recent_otps()
    if not rows:
        return
    
    # Get all uploaded numbers
    all_numbers = set()
    ...
    if not all_numbers:
        logger.warning("⚠️ No numbers uploaded")
        return
    
    logger.info(f"📊 Checking {len(all_numbers)} uploaded numbers")
    ...
    for row in rows:
        try:
            dt = row.get("dt", "")
            number = str(row.get("num", "")).strip().lstrip("+")
            cli = str(row.get("cli", "Unknown")).upper()
            message = str(row.get("message", ""))
            
            if not number or not message:
                continue
            
            # ⚡ Check if number in uploaded list
            if number not in all_numbers:
                logger.debug(f"⏭️ {number} not in pool (skipped)")
                continue
            
            # Check time (last 30 minutes)
            try:
                msg_time = datetime.strptime(dt, "%Y-%m-%d %H:%M:%S")
                if (current_time - msg_time).total_seconds() > 1800:
                    continue
            except:
                pass
            ...
            if not otp_code:
                logger.debug(f"⏭️ {number} - OTP not found in message")
                continue
            ...
            try:
                ...
                logger.info(f"✅ Channel notified")
            except Exception as e:
            ...
            # ═══════════════════════════════════════════════════════
            # INBOX MESSAGE (Full number - complete)
            # Find user who has this number assigned
            # ═══════════════════════════════════════════════════════
            for user_id, user_info in user_numbers.items():
                user_num = str(user_info.get("number", "")).lstrip("+").strip()
                if user_num == number:
                    inbox_msg = (
                        f"🔔 New OTP\n\n"
                        f"📱 App: {cli}\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Admin callbacks ──
        ...
        if user_id != ADMIN_ID:
            ...
            return
        await query.answer("🔐 Logging in to panel...")
        success = await login_panel(context, user_id)
        if success:
            await query.edit_message_text(
                "✅ *Panel Login Successful!*\n\n"
                "🔑 Connected to SMS Panel\n\n"
                "You can now manage numbers",
                parse_mode="Markdown",

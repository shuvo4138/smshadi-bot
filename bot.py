...
JOIN_CHANNEL = os.getenv("JOIN_CHANNEL", "https://t.me/+SWraCXOQrWM4Mzg9").strip()
...
def service_keyboard():
    ...
    for i, s in enumerate(SERVICES):
        ...
        avail = stats[s]["available"]
        row.append(InlineKeyboardButton(f"{emoji} {s} ({avail})", callback_data=f"service_{s}"))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:

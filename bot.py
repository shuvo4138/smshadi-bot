...
def fetch_all_recent_otps():
    """Fetch recent OTPs from Panel API — auto re-login if session expired"""
    ...
    def _fetch():
        ...
        # Full DataTables params matching browser request exactly
        params = {
            "fdate1": fdate1,
            "fdate2": fdate2,
            "frange": "",
        ...
        # Add per-column params (9 columns)
        for i in range(9):
    ...
    try:
        ...
        try:
            ...
            # Not JSON — session expired, re-login
...
async def login_panel_admin(context, user_id: int):
    ...
    ok = panel_login()
    if ok:
        await context.bot.send_message(user_id, "✅ *Panel login successful!*\n\n🔑 OTP fetch ready", parse_mode="Markdown")
    else:
        await context.bot.send_message(user_id, "❌ Panel login failed")
    return ok

...
DASHBOARD_CDR_URL = "http://185.2.83.39/ints/agent/res/data_smscdr.php"
DASHBOARD_SESSKEY = os.getenv("DASHBOARD_SESSKEY", "Q05RR0FRT0hCUA==").strip()
DASHBOARD_LOGIN_URL = "http://185.2.83.39/ints/login"

current_sesskey = DASHBOARD_SESSKEY

def refresh_sesskey():
    """Auto-login to get fresh sesskey"""
    global current_sesskey
    try:
        session = requests.Session()
        resp = session.get(DASHBOARD_LOGIN_URL, headers={"User-Agent": "Mozilla/5.0"}, timeout=10)
        captcha_match = re.search(r'What is (\d+)\s*\+\s*(\d+)', resp.text)
        captcha_answer = str(int(captcha_match.group(1)) + int(captcha_match.group(2))) if captcha_match else "6"
        ...
        match = re.search(r'sesskey[=:]([A-Za-z0-9+/=]+)', login_resp.text)
        if match:
            current_sesskey = match.group(1)
            logger.info(f"✅ New sesskey: {current_sesskey[:10]}...")
            return current_sesskey
        # Try cookie
        if "PHPSESSID" in session.cookies:
            import base64
            current_sesskey = base64.b64encode(session.cookies["PHPSESSID"].encode()).decode()
            logger.info(f"✅ Sesskey from cookie")
            return current_sesskey
    except Exception as e:
        logger.error(f"Sesskey refresh error: {e}")
    return current_sesskey

def fetch_all_recent_otps():
    """Fetch recent OTPs from dashboard CDR API"""
    global current_sesskey
    try:
        now = datetime.now()
        ...
        if resp.status_code == 200:
            ...
            if rows:
                logger.info(f"📡 Dashboard CDR: {len(rows)} rows")
                return rows
            else:
                # Might be expired — try refresh
                logger.warning("⚠️ No data, refreshing sesskey...")
                refresh_sesskey()
                return []
        else:
            logger.error(f"Dashboard CDR HTTP error: {resp.status_code}")
            refresh_sesskey()
    except Exception as e:
        logger.error(f"Dashboard CDR fetch error: {e}")
    return []

def fetch_otp_for_number(number: str):
    """Fetch OTP for specific number from dashboard"""
    try:
        ...
        resp = requests.get(
            ...
            params={
                ...
                "fdate2": fdate2,
                "fnum": clean_num,
                "frange": "", "fclient": "", "fcli": "", "fg": "0",
                "sesskey": DASHBOARD_SESSKEY,
                "sEcho": 1,
                "iDisplayStart": 0,
                "iDisplayLength": 10,
            },
        ...
        if resp.status_code == 200:
            ...
            if rows and len(rows[0]) >= 6:
                ...
                return {
                    ...
                    "sender": str(latest[4] or "Unknown"),
                    "message": str(latest[5] or ""),
                    "number": clean_num,
                }
    except Exception as e:
        logger.error(f"Dashboard fetch_otp error: {e}")
    return None
...
async def poll_otps(context):
    """Background task to check for new OTPs via Dashboard CDR API"""
    global otp_cache
    ...
    for row in rows:
        try:
            ...
            sender = str(row[4] or "Unknown").strip()
            ...
            logger.info(f"✅ OTP! Number: {number} | Sender: {sender} | OTP: {otp_code}")
            ...
            # Notify user if they have this number
            for uid, info in user_numbers.items():
                assigned_num = info["number"].lstrip("+").strip()

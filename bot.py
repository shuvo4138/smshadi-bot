...
def fetch_otp_for_number(number: str):
    ...
    try:
        ...
        logger.info(f"📡 OTP fetch: {resp.status_code}")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🏠 Home":
        ...
        if user_id not in user_numbers:
            ...
            return
        num = user_numbers[user_id]
        await update.message.reply_text(f"🔍 খুঁজছি...", parse_mode="Markdown")
        ...
        if result:
            ...
            await update.message.reply_text("⏳ এখনো OTP আসেনি।", parse_mode="Markdown")
        ...
        if user_id in user_numbers:
            ...
            await update.message.reply_text("❌ কোনো number নেই। আগে number নিন।")
...
async def clear_numbers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_numbers.clear()
    save_numbers()
    await update.message.reply_text("✅ সব clear হয়েছে।")

...
async def poll_otps(context):
    """Background task to check for new OTPs via CR API — auto adds numbers to pool"""
    global otp_cache
    ...
    logger.info(f"🔍 poll_otps: {len(rows)} rows")
    
    for row in rows:
        try:
            number = str(row.get("num", "")).strip().lstrip("+")
            message = str(row.get("message", "")).strip()
            sender = str(row.get("cli", "Unknown")).strip()
            ...
            # Auto-detect service from sender/CLI
            sender_upper = sender.upper()
            if "FACEBOOK" in sender_upper or "FB" in sender_upper:
                service = "Facebook"
            elif "WHATSAPP" in sender_upper or "WA" in sender_upper:
                service = "WhatsApp"
            elif "TIKTOK" in sender_upper:
                service = "TikTok"
            elif "INSTAGRAM" in sender_upper or "IG" in sender_upper:
                service = "Instagram"
            elif "TELEGRAM" in sender_upper:
                service = "Telegram"
            else:
                service = sender  # Unknown sender, use as-is
            
            otp_code = extract_otp(message)
            flag = get_flag(number)
            
            logger.info(f"✅ OTP! Number: {number} | Service: {service} | OTP: {otp_code}")
            ...
            # Notify user if they have this number assigned
            for uid, info in user_numbers.items():
                ...
                if assigned_num == number:
                    try:
                        ...
                        logger.info(f"📬 Notified user {uid} for number {number}")
                    except Exception as e:

    ...
    try:
        ...
        for otp_data in fetch_cr_api_otps():
            try:
                ...
                # SMS থেকে শুধু দরকারি অংশ নেওয়া (garbage text বাদ)
                clean_message = re.sub(r'[A-Za-z0-9+/]{10,}', '', message).strip()
                ...
                quoted_sms = "\n".join(
                    f">{escape_mdv2(line)}" for line in clean_message.splitlines() if line.strip()
                )

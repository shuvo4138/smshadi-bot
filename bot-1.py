...
def add_user(user_id, username):
    ...
    if uid not in users_db:
        ...
        return True
    # backward compat — পুরনো format string ছিল
    if isinstance(users_db[uid], str):
        users_db[uid] = {
            "username": users_db[uid],
            "joined": "2024-01-01"
        }
    return False
...
def get_username(user_id):
    uid = str(user_id)
    val = users_db.get(uid, {})
    if isinstance(val, dict):
        return val.get("username", uid)
    return str(val)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("getcountry:"):
    ...
    elif data.startswith("deletepool:"):
        if user_id != ADMIN_ID:
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ হ্যাঁ, Delete করো", callback_data=f"confirmdeletepool:{pool_key}")],
        ...
        await query.edit_message_text(
            ...
            f"🌍 Pool: *{label}*\n"
            f"📱 Numbers: `{count}`\n\n"
            f"এই pool এর সব number delete হয়ে যাবে!",
            parse_mode="Markdown",
            reply_markup=keyboard
        )

    elif data.startswith("confirmdeletepool:"):
        if user_id != ADMIN_ID:
            await query.answer("❌ Admin only!", show_alert=True)
            return
        pool_key = data.split(":", 1)[1]
        if pool_key in numbers_pool:
            ...
            await query.edit_message_text(
                f"✅ *Pool Deleted!*\n\n"
                f"🌍 Pool: `{pool_key}`\n"
                f"🗑 Removed: `{count}` numbers",
                parse_mode="Markdown"
            )
        else:
            await query.edit_message_text("❌ Pool not found", parse_mode="Markdown")

    elif data == "deletesingle":
        if user_id != ADMIN_ID:
            await query.answer("❌ Admin only!", show_alert=True)
            return
        ...
        await query.edit_message_text(
            ...
            "এখন যে number delete করতে চাও সেটা পাঠাও\n"
            "Format: `91xxxxxxxxxx` (country code সহ, + ছাড়া)",
            parse_mode="Markdown"
        )
        ...
        if action == "stats":
            ...
            pool = get_numbers_pool()
            if not pool:
                await query.edit_message_text("❌ No pools available", parse_mode="Markdown")
                return
            buttons = [
                [InlineKeyboardButton(f"🗑 {get_button_label(pk)} ({len(pool[pk])})", callback_data=f"deletepool:{pk}")]
                for pk in sorted(pool.keys())
            ]
            buttons.append([InlineKeyboardButton("✏️ Delete Single Number", callback_data="deletesingle")])
            await query.edit_message_text(
                "🗑️ *Delete Numbers*\n\n"
                "Pool delete করতে pool এ ক্লিক করো\n"
                "Single number delete করতে নিচের button চাপো:",
                parse_mode="Markdown",
...
async def handle_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Single number delete
    if context.bot_data.get("pending_delete_number"):
        context.bot_data["pending_delete_number"] = False
        number = update.message.text.strip().lstrip("+")
        deleted = False
        ...
        if not deleted:
            await update.message.reply_text(
                f"❌ Number `+{number}` কোনো pool এ পাওয়া যায়নি",
                parse_mode="Markdown"
            )
        return
...
async def post_init(app):
    # Pending updates clear — restart এ পুরনো message process হবে না

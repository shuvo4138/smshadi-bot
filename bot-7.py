...
def panel_login() -> bool:
    ...
    try:
        ...
        # Debug: Print CAPTCHA section
        captcha_section = resp.text[resp.text.find("What is"):resp.text.find("What is")+200] if "What is" in resp.text else "NOT FOUND"
        logger.debug(f"🔍 CAPTCHA section: {captcha_section}")
        
        # Extract CAPTCHA
        a, b = extract_captcha(resp.text)
        if a is None:
            logger.warning("⚠️ CAPTCHA extraction failed - checking alternative patterns...")
            # Try to find any numbers
            numbers = re.findall(r'\d+', resp.text)
            logger.warning(f"⚠️ Found numbers in page: {numbers[:10]}")
            return False
        ...
        logger.info(f"📤 Sending login request with captcha={captcha_answer}...")
        login_resp = s.post(
        ...
        if "PHPSESSID" in s.cookies:
            ...
            logger.info(f"✅ PHPSESSID obtained: {PANEL_PHPSESSID[:10]}...")
            ...
            if sesskey_match:
                ...
                logger.info(f"✅ SESSKEY obtained: {PANEL_SESSKEY[:10]}...")
            
            logger.info(f"✅ Panel login successful!")
            return True
        else:
            logger.error("❌ Panel login failed - no PHPSESSID in cookies")
            logger.error(f"Response status: {login_resp.status_code}")
            return False

...
@app.route('/otp-webhook', methods=['POST'])
def otp_webhook():
    """Receive OTP directly from Panel"""
    try:
        data = request.json
        logger.info(f"🔔 WEBHOOK RECEIVED: {data}")
        
        phone = data.get('phone') or data.get('num') or data.get('number')
        otp = data.get('otp') or data.get('code')
        cli = data.get('cli') or data.get('sender', 'Unknown')
        message = data.get('message') or data.get('sms', '')
        
        if not phone or not otp:
            logger.warning(f"⚠️ Invalid webhook data: {data}")
            return {'status': 'error', 'message': 'Missing phone or OTP'}, 400
        
        phone = str(phone).lstrip('+').strip()
        
        # Extract OTP if not provided
        if not otp and message:
            otp = extract_otp(message)
        
        if not otp:
            logger.warning(f"⚠️ Could not extract OTP from: {message}")
            return {'status': 'error', 'message': 'Could not extract OTP'}, 400
        
        logger.info(f"✅ WEBHOOK OTP: +{phone} | {otp} | {cli}")
        
        # Find user with this number
        user_id = None
        for uid, info in user_numbers.items():
            if str(info.get('number', '')).lstrip('+').strip() == phone:
                user_id = uid
                break
        ...
        if user_id:
            ...
            async def send():
                try:
                    # You need to get the bot instance here
                    # This is a simplified version
                    logger.info(f"📨 Sending OTP to user {user_id}")
                except Exception as e:
                    logger.error(f"❌ Error sending OTP: {e}")
            
            loop.run_until_complete(send())
            
            return {'status': 'success', 'message': 'OTP forwarded'}, 200
        else:
            logger.warning(f"⚠️ No user found for number: +{phone}")
